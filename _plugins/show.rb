module Jekyll
  class ShowDataGenerator < Jekyll::Generator
    ...
    def ignore_missing(show)
      ...
      IGNORE_MISSING_IN_SHOW_TYPES.include? show.data['show_type']
    end
    ...
    def generate_show(show)
      ...
      # If show_type / venue specified store relative path to its page
      if show.data["show_type"]
        show.data["show_type_path"] = "/show_types" + Show_typePage.make_path(show.data["show_type"]) + "/"
      end
      if show.data["venue"]
    ...
    def sort_shows(shows)
      return shows.sort_by do | show |
        if show.data.key?("show_type_sort")
          # Sort by year, then by show_type_sort
          [show.data["year_page"].data["start_year"], show.data["show_type_sort"].to_i]
        else
          # If no show_type_sort, assume 1000
