SORT_PLACES = ['Studios', 'Outside Broadcast', nil]
  ...
  class VenueGenerator < Generator
    ...
    def generate(site)
      ...
      for venue_page in @collection.docs
        ...
        if venue_page.data.key?('place_type')
          venue_page.data['place_type_sort'] = SORT_PLACES.include?(venue_page.data['place_type']) ? venue_page.data['place_type'] : nil
        else
          venue_page.data['place_type_sort'] = nil
        end
      ...
      # Venue groups
      site.data['venues_by_place_type'] = Hash.new
      @collection.docs.each do |venue|
        site.data['venues_by_place_type'][venue.data.fetch('place_type_sort', nil)] ||= Array.new
        site.data['venues_by_place_type'][venue.data.fetch('place_type_sort', nil)].push(venue)
      end
      site.data['venues_by_place_type'].each do |place_type, venues|
